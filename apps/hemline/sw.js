const CACHE_PREFIX = 'hemline-';
const CACHE_NAME = 'hemline-v1.0.0-p2';
const APP_VERSION = '1.0.0';
const RELEASE_DATE = '2026-07-17';
const CACHE_PROTOCOL = 2;
const RELEASE_NOTES = [
  'Первый релиз: локальный PvP и четыре характера офлайн-бота.',
  'Поле 8×8, захват ровно пары, два сдвига и перехват первого камня.',
  'Партия сохраняется локально; приложение работает офлайн и поддерживает Workshop Mode.'
];
const APP_SHELL = [
  './', './index.html', './app.config.json', './styles.css', './app.js', './engine.js', './bot.js',
...
async function precacheFreshShell() {
  ...
  await Promise.all([...new Set(SHELL_KEYS.values())].map(async (canonicalUrl) => cache.put(canonicalUrl, await fetchFresh(canonicalUrl))));
}
async function networkFirstFresh(request, canonicalUrl, fallbackUrl = canonicalUrl) {
  try { const response = await fetchFresh(request); const cache = await caches.open(CACHE_NAME); await cache.put(canonicalUrl, response.clone()); return response; }
  catch { return caches.match(canonicalUrl).then((cached) => cached || caches.match(fallbackUrl)); }
}
