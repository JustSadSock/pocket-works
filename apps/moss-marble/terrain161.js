...
function addSand(m,z,s){const o=organic(z,s,10),i=scaled(o,.87),h=Number(z.surfaceZ??.35)+.08;ring(m,o,i,.24,h,C.soil,0);m.fan(i,()=>h,C.sand,8);const b=bounds(z);for(let k=0;k<12;k++){const x=b.x+b.w*(.13+noise(k,s,73)*.74),y=b.y+b.h*(.13+noise(k,s,79)*.74);disc(m,x,y,h+.12,7+noise(k,s,83)*18,3+noise(k,s,89)*8,k%3?C.sand:C.sand2,8,12,s+k)}}
