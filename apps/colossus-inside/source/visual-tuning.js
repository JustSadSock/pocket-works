...
function chamferedPrism(scene, name, parent, width, depth, thickness, chamfer, y, z, material) {
  ...
  const indices = [];
  for (let i=1;i<7;i++) indices.push(8,8+i,8+i+1);
...
function addArmorTraversal(scene) {
  ...
  const armor = makeMaterial(scene,'faceted-route-armor',new Color3(.115,.145,.138),new Color3(.030,.043,.040),new Color3(.42,.44,.37),72);
  ...
  for(const mesh of scene.meshes){
    ...
    if(/^(back|shoulder|interior|head)-plate-\d+(-ridge)?$/.test(name)||/^spine-fin-\d+$/.test(name)) mesh.visibility=.02;
  }
}
...
function addRouteInlays(scene) {
  ...
  for(const [carrier,route] of Object.entries(ROUTES)){
    ...
    route.pads.forEach(([a,b],index)=>{
      const mid=(a+b)*.5,p=routePoint(carrier,0,mid),depth=Math.max(.35,b-a-.42),edgeX=Math.max(1.1,p.width*.78);
      for(const side of [-1,1]){
        const inlay=MeshBuilder.CreateBox(`route-inlay-${carrier}-${index}-${side}`,{width:.055,height:.018,depth},scene);
        inlay.parent=parent;inlay.position.set(edgeX*side,p.y+.018,mid);inlay.material=index===route.pads.length-1?teal:brass;
      }
    });
  }
}
...
function addInteriorArchitecture(scene) {
  ...
  const shell=makeMaterial(scene,'interior-shell-runtime',new Color3(.15,.175,.16),new Color3(.030,.036,.032));
  ...
  for(let i=0;i<7;i++){
    const z=-10+i*3.65;
    for(const side of [-1,1]){
      const wall=MeshBuilder.CreateBox(`interior-bulkhead-${i}-${side}`,{width:.48,height:6.8,depth:.62},scene);wall.parent=parent;wall.position.set(side*4.75,3.4,z);wall.rotation.z=side*-.14;wall.material=rib;
      const brace=MeshBuilder.CreateBox(`interior-brace-${i}-${side}`,{width:4.2,height:.32,depth:.48},scene);brace.parent=parent;brace.position.set(side*2.9,6.25,z);brace.rotation.z=side*.16;brace.material=rib;
    }
  ...
  for(let i=0;i<6;i++)for(const side of [-1,1]){
    const z=-8.7+i*3.55, lamp=MeshBuilder.CreateBox(`interior-guide-lamp-${i}-${side}`,{width:.20,height:.14,depth:.48},scene);lamp.parent=parent;lamp.position.set(side*3.55,1.25,z);lamp.material=lampMat;
    const light=new PointLight(`interior-guide-light-${i}-${side}`,Vector3.Zero(),scene);light.parent=lamp;light.intensity=.38;light.range=6;light.diffuse=new Color3(.20,.78,.63);
  }
  ...
  const targetLight=new PointLight('stabilizer-world-light',Vector3.Zero(),scene);targetLight.parent=core;targetLight.intensity=1.2;targetLight.range=11;targetLight.diffuse=new Color3(.12,1,.76);
  return{ring,targetLight};
}

export function installColossusVisualTuning(){
  const scene=EngineStore.LastCreatedScene;if(!scene)return;
  globalThis.__PW_VISUAL_TUNING__={ready:true,version:7};
  ...
  const tune=()=>{
    const inside=globalThis.__PW_TEST_STATE__?.carrier==='interior';scene.imageProcessingConfiguration.exposure=inside?1.35:1.29;scene.imageProcessingConfiguration.contrast=inside?1.04:1.08;
    if(inside){scene.fogDensity=Math.min(scene.fogDensity||.006,.006);scene.fogColor=new Color3(.085,.10,.09);cameraLamp.intensity=1.65;cameraLamp.range=19;}else{if(scene.fogDensity>.0028)scene.fogDensity=.0023;scene.fogColor=new Color3(.11,.14,.145);cameraLamp.intensity=1.28;cameraLamp.range=24;}
    for(const m of scene.materials){const name=m.name||'';if(!/^authored-/.test(name))continue;if('ambientColor'in m)m.ambientColor=/interior|bone|edge/i.test(name)?new Color3(.64,.61,.52):new Color3(.50,.52,.48);if('emissiveColor'in m){if(/sensor/i.test(name))m.emissiveColor=new Color3(.035,.32,.26);else if(/heart/i.test(name))m.emissiveColor=new Color3(.10,.014,.010);else if(/player|cloth/i.test(name))m.emissiveColor=new Color3(.085,.065,.040);else if(/armor|bone|interior/i.test(name))m.emissiveColor=new Color3(.045,.055,.050);}}
  };
