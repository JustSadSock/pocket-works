(() => {
  ...
  proto.drawImage = function patchedDrawImage(source, ...args) {
    ...
    try {
      ...
      console.warn('[Miniature Lab] WebGL canvas-filter fallback failed:', error);
      return nativeDrawImage.call(this, source, ...args);
    }
  };
  ...
  function createProgram(gl, vertexSource, fragmentSource) {
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.warn('[Miniature Lab] WebGL link failed:', gl.getProgramInfoLog(program));
      return null;
    }
    return program;
  }
  ...
  function compileShader(gl, type, source) {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      console.warn('[Miniature Lab] WebGL shader failed:', gl.getShaderInfoLog(shader));
      return null;
    }
    return shader;
  }
