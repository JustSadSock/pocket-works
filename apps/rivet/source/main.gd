...
func _build_ui() -> void:
	...
	var start_box: VBoxContainer = start_overlay.get_node("Panel/Margin/Box")
	var mark := Label.new()
	mark.text = "RIVET"
	...
	var pause_box: VBoxContainer = pause_overlay.get_node("Panel/Margin/Box")
	...
	var work_box: VBoxContainer = workshop_overlay.get_node("Panel/Margin/Box")
	...
	var result_box: VBoxContainer = result_overlay.get_node("Panel/Margin/Box")
...
func _draw_player() -> void:
	...
	if invuln > 0.0:
		draw_arc(pos, 38.0 + sin(elapsed * 24.0) * 2.0, 0, TAU, 30, Color(C_PAPER.r, C_PAPER.g, C_PAPER.b, 0.65), 3.0)
...
func _draw_enemy(e: Dictionary) -> void:
	...
	match kind:
		...
		"rammer":
			var ram_color := C_RUST if str(e["state"]) == "charge" else body_color
			draw_circle(pos, 29.0, ram_color)
			draw_line(pos - to_player * 22.0, pos + to_player * 34.0, C_INK, 9.0)
			draw_circle(pos + to_player * 28.0, 8.0, C_WARN if str(e["state"]) == "windup" else C_DARK)
		"drone":
...
func _draw_effect(e: Dictionary) -> void:
	...
	match kind:
		...
		"blast":
			draw_circle(e["pos"], lerpf(68.0, 10.0, t), Color(color.r, color.g, color.b, 0.22 * t))
		...
		"muzzle":
			...
			var p: Vector2 = e["pos"]
			draw_line(p, p + dir * 30.0 * t, color, 8.0 * t + 2.0)
			draw_circle(p + dir * 26.0 * t, 5.0 + 5.0 * t, color)
		"slash":
			var dir: Vector2 = e["dir"]
			var a := dir.angle()
		...
		"beam":
			...
			draw_line(e["a"], e["b"], Color(C_PAPER.r, C_PAPER.g, C_PAPER.b, 0.75 * t), 3.0 * t)
		"telegraph":
			var p: Vector2 = e["pos"]
			var d: Vector2 = e["dir"]
			draw_arc(p, 38.0 + (1.0 - t) * 16.0, 0, TAU, 28, color, 4.0)
			draw_line(p, p + d * 120.0, Color(color.r, color.g, color.b, 0.38 * t), 4.0)
		"telegraph_line":
			var p: Vector2 = e["pos"]
			var d: Vector2 = e["dir"]
			draw_line(p, p + d * 420.0, Color(color.r, color.g, color.b, 0.30 + 0.35 * (1.0 - t)), 8.0 * (1.0 - t) + 2.0)
