...
function enforceWeaponBodyClearance(warrior) {
  ...
  if (bladeLength < 1e-5) return;

  const protectedParts = warrior.getHitSpheres().filter((sphere) =>
    sphere.name === 'chest' || sphere.name === 'abdomen' || sphere.name === 'head'
  );
  ...
  for (const sphere of protectedParts) {
    ...
    if (distance < 1e-5) {
      ...
      normal.scaleInPlace(1 / distance);
    }
