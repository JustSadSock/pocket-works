...
function runImpulse(dx,dy,label){
  ...
  for(let i=0;i<220;i++){
    ...
    assert(Math.abs(pose.upperLength-.34)<.008,`${label}: upper arm stretched`);assert(Math.abs(pose.lowerLength-.33)<.008,`${label}: forearm stretched`);assert(Math.abs(pose.toolLength-1.04)<.008,`${label}: tool stretched`);
...
assert(reports[0].x>.45,'horizontal should sweep laterally');assert(reports[2].y>.4,'overhead should sweep vertically');assert(reports[4].x>.25&&reports[4].y>.25,'diagonal should occupy two axes');
