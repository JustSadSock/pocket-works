...
function simplifyQuestion(text=''){
  let q=tidySpaces(text)
    .replace(/,?\s*(?:за|згідно з) конспектом,?\s*/gi,' ')
    .replace(/\s+(?:у|в) конспекті(?=\?|,|\s)/gi,' ')
    .replace(/\bконспект прямо називає\b/gi,'названо')
    .replace(/^Що названо «([^»]+)»\?$/i,'Що означає «$1»?')
    .replace(/^Що названо ([^?]+)\?$/i,'Що означає $1?')
    .replace(/^Який набір ознак .*? належить саме ([^?]+)\?$/i,'Що характерно для $1?')
    .replace(/^Який набір ознак/i,'Які ознаки')
    ...
    .replace(/^Який часовий проміжок відповідає ([^?]+)\?$/i,'Коли тривав $1?')
    ...
    .replace(/^Який недолік ([^?]+) прямо названо\?$/i,'Який недолік має $1?')
    .replace(/^Які ([^?]+) прямо згадано\?$/i,'Які $1 згадано?')
    .replace(/^З чим конспект пов’язує ([^?]+)\?$/i,'З чим пов’язане $1?')
    .replace(/^Який статус ([^?]+) підкреслює конспект\?$/i,'Який статус мали $1?')
    .replace(/є необхідним доповненням до/gi,'потрібне разом із');
  q=q.replace(/\s{2,}/g,' ').replace(/\s+([?,])/g,'$1').trim();
  return upperFirst(q);
}
...
function simplifyExplain(text=''){
  let value=tidySpaces(text)
    ...
    .replace(/^Саме так конспект відрізняє\s+/i,'Так відрізняють ')
    .replace(/^Конспект прямо називає\s+/i,'Запам’ятай: ')
    .replace(/^Конспект прямо згадує\s+/i,'Запам’ятай: ')
    .replace(/^Конспект прямо визначає\s+/i,'Запам’ятай: ')
    .replace(/^У конспекті саме\s+/i,'Саме ')
...
const remainingVerbose=[...seen].filter(q=>/за конспектом|згідно з конспектом|[ув] конспекті|який набір ознак|необхідним доповненням/i.test(q.q||''));
const longQuestions=[...seen].filter(q=>(q.q||'').length>150);
globalThis.__IEV_PLAIN_LANGUAGE__={questions:seen.size,units:LEARNING_UNITS.length,remainingVerbose:remainingVerbose.length,longQuestions:longQuestions.length};
