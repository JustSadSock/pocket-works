...
const CACHE_NAME = 'facet-face-lab-v1.0.0';
const APP_VERSION = '1.0.0';
const RELEASE_DATE = '2026-07-24';
const CACHE_PROTOCOL = 2;
const RELEASE_NOTES = [
  'Первый выпуск: локальный анализ геометрии лица, контроль качества кадра и диапазон неопределённости.',
  'Фотографии не сохраняются и не отправляются на сервер.'
];
...
self.addEventListener('fetch', (event) => {
  ...
  if (requestUrl.origin !== self.location.origin) {
    if (EXTERNAL_CACHE_HOSTS.has(requestUrl.hostname)) {
      event.respondWith(externalCacheFirst(event.request));
    }
    return;
  }
