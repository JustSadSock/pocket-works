...
const RELEASE_NOTES=[
  'Воины верхней армии больше не переворачиваются вверх ногами при движении вниз.',
  'Разведение бойцов работает внутри отделений и по пространственным секторам вместо полного перебора всей армии.',
  'Тяжёлое выравнивание фронта выполняется десять раз в секунду, а списки бойцов переиспользуются.',
  'На сенсорных устройствах поле рисуется с меньшей внутренней плотностью пикселей без изменения размера интерфейса.',
  'Критические исправления боя загружаются до игрового ядра и действуют с первого кадра.'
];
