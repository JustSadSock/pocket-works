...
const CACHE_NAME='blazon-v5.4.2';
const CACHE=CACHE_NAME;
const FINGERPRINT=new URL(self.location.href).searchParams.get('pw_fp')||'';
const REQUIRED=[
  './','./index.html','./styles.css','./living-battle.css',
  './app.js','./bootstrap.js','./engine.js','./core-engine.js','./progression-engine.js','./combat-clarity.js','./battle-performance.js','./heraldry.js',
...
const RELEASE_NOTES=[
  'Воины обеих армий остаются визуально вертикальными и больше не переворачиваются при движении вниз.',
  'Тяжёлые проходы боевой ясности выполняются реже и используют кэшированные списки бойцов.',
  'Боевой HUD обновляется ограниченно, не вызывая перерасчёт интерфейса каждый кадр.',
  'При высокой нагрузке кадровый планировщик выбирает стабильную частоту вместо постоянных рывков.'
];
