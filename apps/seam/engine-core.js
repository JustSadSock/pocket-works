...
export class AxisGame {
  ...
  static fromJSON(data = {}) {
    ...
    game.reserve = { 1: Number(data.reserve?.[1]) || 0, 2: Number(data.reserve?.[2]) || 0 };
    return game;
  }
  ...
  toJSON() {
    return {
      ...
      reserveEnabled: this.reserveEnabled,
  ...
  deploymentCells(player = this.turn) {
    ...
    return this.homeCells(player).filter((cell) => {
      ...
      return DIRECTIONS.some((direction) => this.valueAt(add(cell, direction)) === player);
    });
  }
  ...
  legalMoves(player = this.turn) {
    ...
    for (const cell of this.deploymentCells(player)) {
      result.push({ selected: [], direction: -1, kind: 'deploy', pushed: [], ejected: [], destinations: [keyOf(cell)] });
    }
    return result;
  }
  ...
  inspectMove(group, directionIndex, player = this.turn) {
    ...
    inline = group.length === 1 || group.slice(0, -1).every((cell, index) => {
  ...
  applyMove(move) {
    ...
    if (move?.kind === 'deploy') {
      const destinationKey = move.destinations?.[0];
      const legal = new Set(this.deploymentCells(player).map(keyOf));
      if (!destinationKey || !legal.has(destinationKey)) return { ok: false, reason: 'illegal-deploy' };
      this.board[destinationKey] = player;
      this.reserve[player] -= 1;
      inspected = { selected: [], direction: -1, kind: 'deploy', pushed: [], ejected: [], destinations: [destinationKey] };
    } else {
      const selectedCells = Array.isArray(move?.selected) ? move.selected.map(parseKey) : [];
      inspected = this.inspectMove(selectedCells, move?.direction, player);
      ...
      for (const cell of pushed) {
        ...
        if (this.onBoard(destination)) this.board[keyOf(destination)] = owner;
          ...
          if (this.crown[owner] !== sourceKey && this.reserveEnabled) this.reserve[owner] += 1;
        }
...
export const STYLE_WEIGHTS = Object.freeze({
  rush: { center: 5.0, crownEdge: 1.0, push: 1.2, eject: 2.0, cohesion: 0.8, mobility: 0.3, protect: 1.3, broadside: 0.25, deploy: 0.4 },
  ram: { center: 1.0, crownEdge: 3.5, push: 4.5, eject: 7.5, cohesion: 1.1, mobility: 0.3, protect: 1.1, broadside: 0.35, deploy: 0.25 },
  shell: { center: 2.1, crownEdge: 2.6, push: 3.2, eject: 5.0, cohesion: 2.2, mobility: 0.8, protect: 3.5, broadside: 1.0, deploy: 1.0 },
  flank: { center: 2.1, crownEdge: 2.7, push: 3.0, eject: 4.8, cohesion: 0.9, mobility: 2.0, protect: 1.8, broadside: 2.8, deploy: 0.7 },
  balanced: { center: 2.3, crownEdge: 2.3, push: 2.8, eject: 4.6, cohesion: 1.6, mobility: 0.8, protect: 2.4, broadside: 0.75, deploy: 0.65 }
});
...
export function evaluatePosition(game, player, style = 'balanced') {
  ...
  const material = (game.cellsFor(player).length + game.reserve[player] * 0.45)
    - (game.cellsFor(opponent).length + game.reserve[opponent] * 0.45);
...
export function chooseAIMove(game, {
  ...
  const scored = moves.map((move) => {
    ...
    if (trial.celsFor(3 - player).length < beforeCount) score += weights.eject * 2.5;
    if (move.kind === 'broadside') score += weights.broadside;
    if (move.kind === 'deploy') score += weights.deploy;
