...
const CACHE_NAME = 'seam-v1.0.0-p2';
const APP_VERSION = '1.0.0';
const RELEASE_DATE = '2026-07-17';
const CACHE_PROTOCOL = 2;
const RELEASE_NOTES = [
  'Первая версия: локальная игра и три уровня компьютерного соперника.',
  'Правила выбраны после 30 конфигураций, 480 сравнительных и 200 финальных бот-партий.',
  'Добавлены сохранение, отмена ходов, процедурный звук, вибрация, полный баланс-аудит и PWA-режим.'
];
const APP_SHELL = [
  './', './index.html', './app.config.json', './styles.css', './app.js', './engine.js', './BALANCE_AUDIT.md', './manifest.webmanifest', './icons/icon.svg',
...
self.addEventListener('fetch', (event) => { if (event.request.method !== 'GET') return; const url = new URL(event.request.url); if (url.origin !== self.location.origin) return; if (event.request.mode === 'navigate') { event.respondWith(networkFirstFresh(event.request, SCOPE_URL.href, SCOPE_URL.href)); return; } const canonical = SHELL_KEYS.get(url.pathname); if (canonical) event.respondWith(networkFirstFresh(event.request, canonical, SCOPE_URL.href)); });
