...
const APP_SHELL=[
  './','./index.html','./app.config.json','./styles.css','./app.js','./sim-core.mjs','./manifest.webmanifest','./icons/icon.svg','./README.md',
...
self.addEventListener('fetch',event=>{if(event.request.method!=='GET')return;const u=new URL(event.request.url);if(u.origin!==self.location.origin)return;if(event.request.mode==='navigate'){event.respondWith(networkFirstFresh(event.request,SCOPE_URL.href,SCOPE_URL.href));return}const canonical=SHELL_KEYS.get(u.pathname);if(canonical)event.respondWith(networkFirstFresh(event.request,canonical,SCOPE_URL.href))});
