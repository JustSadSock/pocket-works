...
function addConstraint(type, bodyA, bodyB = null, options = {}) {
  ...
  state.constraints.push({
    ...
    damping: options.damping ?? (type === 'rope' ? .04 : .12), tension: 0
  });
}
...
function loadScene(scene, record = true) {
  ...
  if (scene === 'bridge') {
    ...
    for (let i = 0; i < count; i++) {
      ...
      planks.push(addBody('box', x, y, { w: 40, h: 14, material: 'wood' }));
    }
    addConstraint('rope', planks[0], null, { bx: left.x, by: y - 55, rest: Math.hypot(planks[0].x-left.x,55) });
    for (let i = 0; i < planks.length - 1; i++) addConstraint('rope', planks[i], planks[i + 1], { rest: 42 });
    addConstraint('rope', planks.at(-1), null, { bx: right.x, by: y - 55, rest: Math.hypot(planks.at(-1).x-right.x,55) });
    ...
    for (let i = 0; i < count; i++) {
      ...
      const displaced = i === 0;
      const ball = addBody('circle', displaced ? x - 70 : x, displaced ? ballY - 35 : ballY, { r: 17, material: 'steel' });
      addConstraint('rope', ball, null, { bx: x, by: anchorY, rest: ballY - anchorY });
    }
...
function openMaterials() {
  ...
  openSheet('СВОЙСТВА ТЕЛА', 'МАТЕРИАЛ', html, () => {
    sheetBody.querySelectorAll('[data-material]').forEach(button => button.addEventListener('click', () => {
      ...
      if (selected) { pushHistory(); selected.material = state.material; refreshBodyPhysics(selected); }
...
function materialDescription(key) {
  return {
    wood: 'средний вес, цепкое, спокойно гасит удары', steel: 'очень тяжёлое и почти не отскакивает', rubber: 'упругое, цепкое и хаотично живое', ice: 'лёгкое и почти без трения', foam: 'очень лёгкая, падает медленно'
  }[key];
}

function openMenu() {
  const html = `<div class="menu-list">
    <button class="menu-row" type="button" data-action="scenes"><svg viewBox="0 0 24 24"><path d="M4 19V8l8-4 8 4v11M8 19v-6h8v6"/></svg><span><b>СЦЕНЫ</b><small>мост, орбита, маятники или чистый стол</small></span></button>
    ...
    ${menuToggle('sound','ЗВУК','удары и связи',state.sound,'M5 10v4h4l5 4V6l-5 4H5ZM17 9c1.4 1.8 1.4 4.2 0 6')}
...
function openScenes() {
  const scenes = [
    ...
    ['orbit','02','ОРБИТА','рой тел вокруг управляемого поля'],
    ['cradle','03','МАЯТНИКИ','передача импульса по цепочке'],
    ['empty','04','ЧИСТЫЙ СТОЛ','никаких оправданий, только физика']
  ];
...
function exportWorld() {
  const payload = JSON.stringify({ app: 'impuls', version: 1, savedAt: new Date().toISOString(), world: snapshotWorld() }, null, 2);
...
function duplicateSelected() {
  ...
  const copy = createBody(body.shape, clamp(body.x + 24, 20, state.worldWidth - 20), clamp(body.y - 24, 20, state.worldHeight - 20), {
    r: body.r, w: body.w, h: body.h, angle: body.angle + .08, material: body.material, pinned: false, vx: 80, vy: -80
  });
