const CACHE_PREFIX = 'petlya-';
const CACHE_NAME = 'petlya-v1.0.0';
const APP_VERSION = '1.0.0';
const RELEASE_DATE = '2026-07-24';
const CACHE_PROTOCOL = 2;
const RELEASE_NOTES = [
  '18 временных головоломок в трёх главах.',
  'Запись маршрутов, до трёх синхронных эхо, печати и затворы.',
  'Архив, сохранения, звук, вибрация и полная офлайн-работа.'
];
...
async function fetchFresh(input) {
  ...
  if (!response?.ok) throw new Error(`Fresh PETLYA request failed: ${response?.status || 'network'}`);
  return response;
}
