const CACHE_PREFIX = 'bubble-bob-';
const CACHE_NAME = 'bubble-bob-v1.2.0';
const APP_VERSION = '1.2.0';
const RELEASE_DATE = '2026-07-26';
const CACHE_PROTOCOL = 2;
const RELEASE_NOTES = [
  'Добавлены опасные уклонения: близкий проход мимо медузы даёт очки и заряд волны.',
  'Сделан более честный спавн без наложения объектов и невозможных стартовых связок.',
  'Комбо теперь имеет короткое окно прощения и не рушится от одного случайного промаха.',
  'Добавлены новые задания на риск и очки, повышена награда за выполнение смены.'
];
const APP_SHELL = [
  './','./index.html','./app.config.json','./styles.css','./app.js','./game-core.js','./game-draw.js',
  ...
  '../../shared/workshop-mode.css','../../shared/workshop-mode.js','../../shared/capabilities/motion.js',
  '../../shared/capabilities/storage.js','../../shared/capabilities/transfer.js','../../shared/capabilities/audio.js',
...
async function fetchFresh(input){const response=await fetch(buildNetworkUrl(input),{cache:'no-store',credentials:'same-origin',redirect:'follow'});if(!response||!response.ok)throw new Error(`Fresh application request failed: ${response?.status||'network'}`);return response}
async function precacheFreshShell(){const cache=await caches.open(CACHE_NAME);await Promise.all([...new Set(SHELL_KEYS.values())].map(async canonicalUrl=>{const response=await fetchFresh(canonicalUrl);await cache.put(canonicalUrl,response)}))}
