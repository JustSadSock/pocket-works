const CACHE_PREFIX = 'ottisk-';
const CACHE_NAME = 'ottisk-v1.0.0';
const APP_VERSION = '1.0.0';
const RELEASE_DATE = '2026-07-22';
const CACHE_PROTOCOL = 2;
const RELEASE_NOTES = [
  'Три движущихся ризографических слоя и восемь оттисков в каждом тираже.',
  'Серии точных совмещений, локальные рекорды и открываемые палитры.',
  'Процедурная графика, звук, вибрация и полноценная офлайн-работа.'
];
...
async function fetchFresh(input) {
  ...
  if (!response || !response.ok) throw new Error(`Fresh ОТТИСК request failed: ${response?.status || 'network'}`);
  return response;
}
