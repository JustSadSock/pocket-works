...
function makeMaterial(scene, name, color, roughness = 0.82) {
...
function solveKnee(hip, foot, forward, side) {
  const delta = foot.subtract(hip);
  const rawD = delta.length();
  const d = clamp(rawD, 0.16, UPPER + LOWER - 0.025);
  const dir = delta.scale(1 / (rawD || 1));
  const a = (UPPER * UPPER - LOWER * LOWER + d * d) / (2 * d);
  const h = Math.sqrt(Math.max(0, UPPER * UPPER - a * a));
  let bend = forward.subtract(dir.scale(Vector3.Dot(forward, dir)));
  if (bend.lengthSquared() < 1e-4) bend = new Vector3(side * 0.05, 0, 1);
  bend.normalize();
...
export class HumanoidRig {
  ...
  makeFootState(side, offset) {
    return {
      side, offset, swinging: false, initialized: false,
      plant: new Vector3(), swingStart: new Vector3(), swingEnd: new Vector3(), lastNorm: 0
    };
  }
  ...
  buildRig(shadowCasters) {
    const linen = makeMaterial(this.scene, 'linen', new Color3(0.37, 0.33, 0.25), 0.95);
    const cloth = makeMaterial(this.scene, 'cloth', new Color3(0.58, 0.48, 0.33), 0.93);
    const skin = makeMaterial(this.scene, 'skin', new Color3(0.55, 0.34, 0.22), 0.82);
    const boot = makeMaterial(this.scene, 'boot', new Color3(0.14, 0.105, 0.075), 0.84);
    ...
    const head = this.linkedNode('head', headBone, spine);
    ...
    shoulderL.position.set(-0.27, 0.26, 0);
    shoulderR.position.set(0.27, 0.26, 0);
    armL.position.y = -0.18;
    armR.position.y = -0.18;

    this.addMesh(MeshBuilder.CreateBox('pelvis-mesh', { width: 0.35, height: 0.2, depth: 0.2 }, this.scene), pelvis, cloth, shadowCasters);
    this.addMesh(MeshBuilder.CreateCylinder('torso-mesh', { height: 0.55, diameterTop: 0.36, diameterBottom: 0.29, tessellation: 10 }, this.scene), spine, linen, shadowCasters);
    ...
    for (const [side, upperNode, lowerNode, footNode] of [
      ...
      const upperMesh = MeshBuilder.CreateCylinder(`thigh-${side}`, { height: 1, diameterTop: 0.16, diameterBottom: 0.13, tessellation: 9 }, this.scene);
      const lowerMesh = MeshBuilder.CreateCylinder(`shin-${side}`, { height: 1, diameterTop: 0.135, diameterBottom: 0.105, tessellation: 9 }, this.scene);
      const footMesh = MeshBuilder.CreateBox(`boot-${side}`, { width: 0.15, height: 0.095, depth: 0.31 }, this.scene);
      this.addMesh(upperMesh, upperNode, cloth, shadowCasters);
      this.addMesh(lowerMesh, lowerNode, cloth, shadowCasters);
      this.addMesh(footMesh, footNode, boot, shadowCasters);
  ...
  update(controller, dt) {
    const pos = controller.localPosition;
    ...
    const bob = Math.sin(controller.gait * 2) * 0.018 * speedNorm;
    this.nodes.pelvis.position.y = 0.97 + bob - slopeNorm * 0.015;
    this.nodes.spine.rotation.x = -controller.lastSlope * 0.17 * speedNorm - speedNorm * 0.025;
    const armSwing = Math.sin(controller.gait) * 0.42 * speedNorm;
    ...
    const leftLanding = this.updateLeg(this.footState.left, controller, forward);
    const rightLanding = this.updateLeg(this.footState.right, controller, forward);
    return [leftLanding, rightLanding].filter(Boolean);
  }

  updateLeg(state, controller, forward) {
    const sideName = state.side < 0 ? 'left' : 'right';
    const legNode = this.nodes[state.side < 0 ? 'legL' : 'legR'];
    const kneeNode = this.nodes[state.side < 0 ? 'kneeL' : 'kneeR'];
    const footNode = this.nodes[state.side < 0 ? 'footL' : 'footR'];
    const lateral = rotateXZ(state.side * 0.145, 0.01, controller.bodyYaw);
    const hip = controller.localPosition.add(new Vector3(lateral.x, 0.95, lateral.z));

    const norm = ((controller.gait + state.offset) % (Math.PI * 2) + Math.PI * 2) % (Math.PI * 2) / (Math.PI * 2);
    const swinging = norm < 0.42 && controller.speed > 0.13;
    if (!state.initialized) {
      state.initialized = true;
      state.plant.copyFrom(this.makeFootCandidate(controller, state.side, -0.05));
    ...
    if (swinging && !state.swinging) {
      state.swingStart.copyFrom(state.plant);
      const stride = 0.26 + clamp(controller.speed / 3.25, 0, 1) * 0.32;
    ...
    if (!swinging && state.swinging) {
      ...
      state.plant.y = terrainHeight(gx, gz) + 0.035;
    ...
    let foot;
    if (swinging) {
      const t = clamp(norm / 0.42, 0, 1);
      const ease = t * t * (3 - 2 * t);
      foot = Vector3.Lerp(state.swingStart, state.swingEnd, ease);
      foot.y += Math.sin(t * Math.PI) * (0.11 + controller.lastSlope * 0.07);
    } else {
      foot = state.plant.clone();
      const gx = foot.x + controller.worldOffsetX;
      const gz = foot.z + controller.worldOffsetZ;
      foot.y = terrainHeight(gx, gz) + 0.032;
      state.plant.y = foot.y;
    }

    const knee = solveKnee(hip, foot.add(new Vector3(0, 0.05, -0.04)), forward, state.side);
    orientYAxis(legNode, hip, knee);
    orientYAxis(kneeNode, knee, foot.add(new Vector3(0, 0.055, 0)));
    legNode.scaling.x = 1; legNode.scaling.z = 1;
    kneeNode.scaling.x = 1; kneeNode.scaling.z = 1;
    footNode.position.copyFrom(foot);
    const gx = foot.x + controller.worldOffsetX;
    const gz = foot.z + controller.worldOffsetZ;
    const n = terrainNormal(gx, gz);
    const tangentForward = forward.subtract(new Vector3(n.x, n.y, n.z).scale(Vector3.Dot(forward, new Vector3(n.x, n.y, n.z)))).normalize();
    footNode.rotationQuaternion = Quaternion.FromLookDirectionLH(tangentForward, new Vector3(n.x, n.y, n.z));
    this.debugTargets[state.side < 0 ? 0 : 1].position.copyFrom(foot);
    return landing;
  }
  ...
  makeFootCandidate(controller, side, forwardOffset) {
    ...
    return new Vector3(x, terrainHeight(gx, gz) + 0.035, z);
  }

  setDebugTargets(enabled) {
    for (const sphere of this.debugTargets) sphere.isVisible = enabled;
  }
