...
export class SiroccoGame {
  constructor(canvas) {
    this.canvas = canvas; this.engine = null; this.scene = null; this.running = false; this.paused = false;
    this.orientationBlocked = false; this.lastFps = 60; this.saveClock = 0; this.pelvisRoll = 0; this.pelvisLift = 0;
  }
  ...
  async init(report = () => {}) {
    ...
    this.scene = new Scene(this.engine); this.scene.skipPointerMovePicking = true; this.scene.autoClear = true;
    await nextFrame();
    ...
    // Quality can alter tessellation, so place the body on the final mesh surface after chunk setup.
    ...
    this.rig.update(this.controller, 1 / 60); this.adaptBodyToFeet(1 / 60); this.camera.update(this.controller, 1 / 60);
    this.scene.render(); await nextFrame();

    this.bindLifecycle(); report('Готово', 1); this.running = true; this.engine.runRenderLoop(() => this.frame());
  }

  restoreSession() {
    try {
      const stored = localStorage.getItem(SESSION_KEY); if (!stored) return;
      const snapshot = JSON.parse(stored);
      if (snapshot?.schema !== SESSION_SCHEMA) { localStorage.removeItem(SESSION_KEY); return; }
      this.controller.restore(snapshot);
    } catch (error) {
      localStorage.removeItem(SESSION_KEY); console.warn('[SIROCCO] Ignoring invalid saved session.', error);
    }
  }
  ...
  bindLifecycle() {
    ...
    this.onVisibility = () => { if (document.hidden) { this.saveSession(); this.audio?.ctx?.suspend?.(); } };
    this.onPageHide = () => this.saveSession();
    window.addEventListener('resize', this.onResize); document.addEventListener('visibilitychange', this.onVisibility); window.addEventListener('pagehide', this.onPageHide);
  }

  handleRebase(dx, dz, offsetX, offsetZ) {
    this.world.setOrigin(offsetX, offsetZ); this.rig.shiftOrigin(dx, dz); this.footprints.shiftOrigin(dx, dz); this.slips.shiftOrigin(dx, dz);
  }
  ...
  adaptBodyToFeet(dt) {
    ...
    const liftTarget = clamp((averageFootY - this.controller.localPosition.y - 0.025) * 0.42, -0.035, 0.055);
    const rollTarget = clamp((right.plant.y - left.plant.y) * 0.22, -0.075, 0.075);
    this.pelvisLift = damp(this.pelvisLift, liftTarget, 10, dt); this.pelvisRoll = damp(this.pelvisRoll, rollTarget, 9, dt);
    this.rig.nodes.pelvis.position.y += this.pelvisLift; this.rig.nodes.pelvis.rotation.z = this.pelvisRoll; this.rig.nodes.spine.rotation.z = -this.pelvisRoll * 0.34;
  }

  applyQuality(preset) {
    if (!preset || !this.engine) return;
    this.engine.setHardwareScalingLevel(preset.hardwareScaling);
    this.world?.setQuality(preset); this.lighting?.setQuality(preset); this.atmosphere?.setQuality(preset);
    this.particles?.setBudget(preset.particles); this.footprints?.setLimit(preset.footprintLimit); this.slips?.setQuality(preset);
  ...
  frame() {
    ...
    if (active) {
      const move = this.input.getMove(); this.controller.applyLook(this.input.consumeLook());
      const groundState = this.controller.update(dt, move);
      const chunksChanged = this.world.update(this.controller.globalX, this.controller.globalZ); if (chunksChanged) this.lighting.registerWorld(this.world);
      const landings = this.rig.update(this.controller, dt); this.adaptBodyToFeet(dt);
      ...
      this.camera.update(this.controller, dt); this.audio.update(this.controller.speed, this.controller.lastSlope);
      this.lastFps = this.engine.getFps(); this.quality.update(dt, this.lastFps); this.debug.update(dt, this.controller, this.quality.preset);
      this.saveClock += dt; if (this.saveClock >= 2.5) { this.saveClock = 0; this.saveSession(); }
    }
    this.scene.render();
  }
  ...
  dispose() {
    ...
    window.removeEventListener('resize', this.onResize); document.removeEventListener('visibilitychange', this.onVisibility); window.removeEventListener('pagehide', this.onPageHide);
    this.input?.dispose(); this.debug?.dispose(); this.particles?.dispose(); this.slips?.dispose(); this.footprints?.dispose(); this.rig?.dispose(); this.world?.dispose(); this.materials?.dispose(); this.atmosphere?.dispose(); this.lighting?.dispose(); this.camera?.dispose(); this.scene?.dispose(); this.engine?.dispose();
  }
}
