const CACHE_PREFIX = 'scenograph-';
const CACHE_NAME = 'scenograph-v0.1.0-p2';
const APP_VERSION = '0.1.0';
const RELEASE_DATE = '2026-09-22';
const CACHE_PROTOCOL = 2;
const RELEASE_NOTES = [
  'Семь теневых миниатюр с точной настройкой света и силуэтов, подсказками и сохранением прогресса.',
  'Свободная мастерская позволяет создавать композиции и сохранять их как PNG-открытки.',
  'Приложение устанавливается на устройство и работает офлайн после первой загрузки.'
];
const APP_SHELL = [
  './',
  './index.html',
...
async function networkFirstFresh(request, canonicalUrl, fallbackUrl = canonicalUrl) {
  ...
  try {
    ...
    return (await cache.match(canonicalUrl)) || cache.match(fallbackUrl);
  }
}
