...
def district_frame(name,z,material,accent,stone,iron,glass):
    """A repeated civic transition motif that visually stitches districts together.

    Everything stays outside the central walking lane or above head height, so it
    improves wayfinding without introducing new collision traps.
    """
    for side in (-1,1):
        x=side*4.72
        box(f'{name}_Pylon_{side}',(x,1.65,z),(.58,3.3,.7),material,bevel_amount=.09)
        box(f'{name}_Band_{side}',(x,2.25,z-.02),(.66,.22,.82),accent,bevel_amount=.035)
        sphere(f'{name}_Signal_{side}',(x,3.32,z),(.17,.22,.17),glass,14,8)
        cylinder(f'{name}_Foot_{side}',(x,.24,z),.38,.48,stone,18)
    box(f'{name}_Lintel',(0,4.35,z),(9.9,.34,.62),iron,bevel_amount=.06)
    curve_between(f'{name}_Cable',(-4.48,4.52,z),(4.48,4.18,z+.16),.045,accent)
...
def main():
    ...
    soot=painted_material('Plaster_FoundrySoot',(.16,.12,.105),.94,seed=15,accent=(.29,.19,.12))
    ...
    for i,z in enumerate(range(-62,49,8)):
        road_mat = stone if z < 4 else (soot if z < 23 else slate if z > 34 else stone)
        box(f'RoadStone_{i}',(0,-.18,z),(10.8,.36,8.15),road_mat,bevel_amount=.09)
        if i%3==0:
    ...
    # District wayfinding is embedded into the pavement: warm Market, hot Foundry,
    # teal Service quarter and pale resonant Tower. It repeats the city's materials
    # instead of relying on floating HUD arrows.
    district_marks=[(-38,saffron,verd),(-12,crimson,copper),(21,verd,iron),(38,ceramic,verd)]
    for mark_index,(z,primary,secondary) in enumerate(district_marks):
        for stripe,x in enumerate((-3.35,-1.1,1.1,3.35)):
            box(f'DistrictInlay_{mark_index}_{stripe}',(x,.035,z),(1.55,.035,.28),primary if stripe%2==0 else secondary,bevel_amount=.018)
        for side in (-1,1):
            box(f'DistrictChevron_{mark_index}_{side}',(side*4.45,.06,z+1.15),(.65,.035,1.45),secondary,rot_y=side*.32,bevel_amount=.02)
    ...
    # Civic frames make each district transition feel like part of one engineered city.
    district_frame('MarketThreshold',-38,ochre,saffron,stone,iron,glass)
    district_frame('FoundryThreshold',-12,terra,crimson,stone,iron,glass)
    district_frame('ServiceThreshold',21,slate,verd,stone,iron,glass)
    district_frame('TowerThreshold',38,slate,ceramic,stone,copper,glass)

    # Two high pedestrian bridges and their hanging utilities imply a city continuing
    # beyond the playable street while preserving more than four metres of clearance.
    for bridge_index,(z,mat) in enumerate([(-6,wood),(29,slate)]):
        box(f'BridgeSpan_{bridge_index}',(0,5.35,z),(11.5,.34,2.25),mat,bevel_amount=.08)
        box(f'BridgeRailA_{bridge_index}',(0,6.02,z-.94),(11.5,.72,.09),iron,bevel_amount=.025)
        box(f'BridgeRailB_{bridge_index}',(0,6.02,z+.94),(11.5,.72,.09),iron,bevel_amount=.025)
        for side in (-1,1):
            curve_between(f'BridgeDrop_{bridge_index}_{side}',(side*4.8,5.18,z),(side*4.65,3.55,z+.48),.055,copper if bridge_index==0 else verd)
        curve_between(f'BridgeUtility_{bridge_index}',(-5.0,4.9,z-.65),(5.0,4.55,z-.42),.07,verd)
    ...
    for side in (-1,1):
        curve_between(f'MarketGarland_{side}',(side*4.7,3.6,-34),(side*4.5,3.1,-25),.035,copper)
        for k,z in enumerate((-33,-30,-27)):
            sphere(f'MarketGlow_{side}_{k}',(side*4.62,3.35,z),(.11,.15,.11),glass,12,6)
    ...
    for brace,z in enumerate((6,11,16,20)):
        box(f'FoundryStreetBrace_{brace}',(-4.7,2.3,z),(.28,4.6,.55),iron,rot_y=.03*(brace%2),bevel_amount=.05)
        curve_between(f'FoundryStreetPipe_{brace}',(-4.55,3.7,z),(-4.55,1.0,z+1.5),.08,copper)
    ...
    for z in (25,31,37):
        ...
        for side in (-1,1): sphere(f'ServiceBeacon_{z}_{side}',(side*3.82,1.92,z),(.13,.18,.13),glass,12,6)
    ...
    for z in (41.5,44.5,47.5):
        for side in (-1,1): sphere(f'TowerResonanceMarker_{side}_{z}',(side*4.65,2.8,z),(.16,.24,.16),ceramic,14,8)
