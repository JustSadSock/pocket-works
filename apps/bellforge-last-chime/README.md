...
## 1.1.1 release recovery

Version 1.1.1 intentionally creates a fresh application version, cache namespace and production fingerprint after the Pocket Works iOS release-convergence fix. This makes the post-polish build unambiguous to installed PWAs: a restored or previously cached 1.0/1.1 client is treated as stale and converges onto the current production release instead of continuing to run an older precache.
