...
const APP_VERSION='1.0.0'; const CACHE_PREFIX='aetherwing-';
...
self.addEventListener('message',(event)=>{if(event.data?.type==='GET_UPDATE_INFO')event.ports?.[0]?.postMessage({version:APP_VERSION,releaseDate:'2026-09-08',releaseNotes:['Инерционный полёт и аэродинамика дракона.','Blender-риг с несколькими полётными клипами.','Процедурный мир, вода, флора, атмосфера и мобильная камера.']});if(event.data?.type==='SKIP_WAITING')self.skipWaiting();});
