...
import { GroundDetailLayer } from './ground-detail';
...
const state=makeFlightState();const controls=createControls(touch);const audio=new FlightAudio();const world=new WorldStreamer(scene);const dragon=new DragonRig(scene);const fauna=new FaunaSystem(scene);const clouds=new CloudLayer(scene);const groundDetail=new GroundDetailLayer(scene);
