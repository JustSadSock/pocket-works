...
function biomeColor(x:number,z:number,h:number){
  const m=moistureAt(x,z),t=temperatureAt(x,z,h);let c:Color4;
  if(h>220)c=new Color4(.46,.48,.43,1);
  else if(h>160)c=new Color4(.50,.54,.39,1);
  else if(m>.73&&t>.35)c=new Color4(.25,.48,.27,1);
  else if(m>.50)c=new Color4(.38,.57,.30,1);
  else if(t<.34)c=new Color4(.42,.51,.37,1);
  else c=new Color4(.54,.64,.33,1);
  const v=(fbm(x*.013+241,z*.013-107)-.5)*.13;
  return new Color4(clamp(c.r+v,0,1),clamp(c.g+v*.85,0,1),clamp(c.b+v*.42,0,1),1);
}
...
export class WorldStreamer{
  ...
  constructor(scene:Scene){
    this.scene=scene;
    this.terrainMaterial=new StandardMaterial('terrainMat',scene);this.terrainMaterial.diffuseColor=Color3.White();this.terrainMaterial.specularColor=new Color3(.025,.032,.018);this.terrainMaterial.specularPower=18;this.terrainMaterial.backFaceCulling=false;this.terrainMaterial.emissiveColor=new Color3(.055,.070,.035);
    ...
    this.fallbackFir=this.makeFir('fallbackFir',new Color3(.15,.38,.20));this.fallbackBroad=this.makeBroad('fallbackBroad',new Color3(.30,.50,.22));this.fallbackRock=this.makeRock('fallbackRock');this.fallbackFlower=this.makeFlower('fallbackFlower');
  ...
  private createChunk(cx:number,cz:number,near:boolean){
    ...
    const root=new TransformNode(`chunk_${cx}_${cz}`,this.scene);const grid=near?(this.quality>.8?29:24):15;const positions:number[]=[],indices:number[]=[],normals:number[]=[],colors:number[]=[];const baseX=cx*CHUNK,baseZ=cz*CHUNK;
  ...
  private createRiver(cx:number){const seg=30;const positions:number[]=[],indices:number[]=[];const x0=cx*CHUNK-HALF_CHUNK;for(let i=0;i<seg;i++){const x=x0+i/(seg-1)*CHUNK;const z=riverCenter(x);const h=terrainHeight(x,z)+3.2;const half=30+noise(x*.01,cx)*18;positions.push(x,h,z-half,x,h,z+half);if(i<seg-1){const a=i*2;indices.push(a,a+2,a+1,a+1,a+2,a+3);}}const mesh=new Mesh(`river_${cx}`,this.scene);const vd=new VertexData();vd.positions=positions;vd.indices=indices;vd.applyToMesh(mesh);mesh.material=this.water;mesh.alwaysSelectAsActiveMesh=false;mesh.receiveShadows=false;mesh.isPickable=false;return mesh;}
  ...
  private scatter(cx:number,cz:number,out:InstancedMesh[]){
    const count=Math.floor(145*this.quality),baseX=cx*CHUNK,baseZ=cz*CHUNK;
    for(let i=0;i<count;i++){const r=hash(cx*97+i,cz*131-i),r2=hash(cx*173-i,cz*67+i);const x=baseX+(r-.5)*CHUNK,z=baseZ+(r2-.5)*CHUNK,h=terrainHeight(x,z),m=moistureAt(x,z);if(Math.abs(z-riverCenter(x))<38||lakeDistance(x,z)<16||h>235)continue;const choice=hash(i+cx*11,cz*17);const template=(choice<.50?(this.templates.fir||this.fallbackFir):choice<.90?(this.templates.broad||this.fallbackBroad):(this.templates.rock||this.fallbackRock));const inst=template.createInstance(`veg_${cx}_${cz}_${i}`);inst.position.set(x,h,z);const s=.78+hash(i*3+cx,cz-i*9)*1.70;inst.scaling.setAll(s);inst.rotation.y=hash(cx-i,cz+i)*Math.PI*2;if(template!==this.fallbackRock&&m<.28)inst.scaling.scaleInPlace(.72);out.push(inst);}
    const flowers=Math.floor(105*this.quality);for(let i=0;i<flowers;i++){const x=baseX+(hash(cx*233+i,cz*181-i)-.5)*CHUNK,z=baseZ+(hash(cx*199-i,cz*239+i)-.5)*CHUNK,h=terrainHeight(x,z),m=moistureAt(x,z);if(h>175||m<.28||Math.abs(z-riverCenter(x))<24||lakeDistance(x,z)<8)continue;const inst=this.fallbackFlower.createInstance(`flower_${cx}_${cz}_${i}`);inst.position.set(x,h+.05,z);const s=.7+hash(i*17+cx,cz*13-i)*1.25;inst.scaling.setAll(s);inst.rotation.y=hash(cx+i*5,cz-i*3)*Math.PI*2;out.push(inst);}
  }
  private makeFir(name:string,color:Color3){const trunk=MeshBuilder.CreateCylinder(`${name}_trunk`,{height:7,diameterTop:.55,diameterBottom:.9,tessellation:7},this.scene);const crown=MeshBuilder.CreateCylinder(`${name}_crown`,{height:9,diameterTop:.3,diameterBottom:5.2,tessellation:8},this.scene);crown.position.y=5.4;const tm=new StandardMaterial(`${name}m`,this.scene);tm.diffuseColor=new Color3(.30,.18,.09);trunk.material=tm;const cm=new StandardMaterial(`${name}c`,this.scene);cm.diffuseColor=color;cm.emissiveColor=color.scale(.08);crown.material=cm;const merged=Mesh.MergeMeshes([trunk,crown],true,true,undefined,false,true)!;merged.name=name;return merged;}
  private makeBroad(name:string,color:Color3){const trunk=MeshBuilder.CreateCylinder(`${name}_trunk`,{height:6,diameterTop:.7,diameterBottom:1.15,tessellation:8},this.scene);const crown=MeshBuilder.CreateIcoSphere(`${name}_crown`,{radius:3.6,subdivisions:2},this.scene);crown.scaling.y=1.25;crown.position.y=5.5;const tm=new StandardMaterial(`${name}m`,this.scene);tm.diffuseColor=new Color3(.31,.18,.10);trunk.material=tm;const cm=new StandardMaterial(`${name}c`,this.scene);cm.diffuseColor=color;cm.emissiveColor=color.scale(.07);crown.material=cm;const merged=Mesh.MergeMeshes([trunk,crown],true,true,undefined,false,true)!;merged.name=name;return merged;}
  private makeRock(name:string){const rock=MeshBuilder.CreateIcoSphere(name,{radius:2.2,subdivisions:2},this.scene);rock.scaling.set(1.7,.7,1.15);const m=new PBRMaterial(`${name}m`,this.scene);m.albedoColor=new Color3(.39,.41,.37);m.roughness=.95;rock.material=m;return rock;}
  private makeFlower(name:string){const stem=MeshBuilder.CreateCylinder(`${name}_stem`,{height:1.25,diameter:.12,tessellation:5},this.scene);stem.position.y=.62;const bloom=MeshBuilder.CreateIcoSphere(`${name}_bloom`,{radius:.38,subdivisions:1},this.scene);bloom.position.y=1.35;const sm=new StandardMaterial(`${name}_sm`,this.scene);sm.diffuseColor=new Color3(.22,.47,.18);stem.material=sm;const bm=new StandardMaterial(`${name}_bm`,this.scene);bm.diffuseColor=new Color3(.92,.61,.18);bm.emissiveColor=new Color3(.12,.065,.012);bloom.material=bm;const merged=Mesh.MergeMeshes([stem,bloom],true,true,undefined,false,true)!;merged.name=name;return merged;}
}
