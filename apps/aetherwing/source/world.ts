...
function biomeColor(x:number,z:number,h:number){
  ...
  if(h>230)c=new Color4(.49,.48,.44,1);
  else if(h>178)c=new Color4(.45,.47,.35,1);
  else if(riverDist<49)c=new Color4(.40,.41,.27,1);
  else if(riverDist<100)c=new Color4(.43,.49,.29,1);
  else if(forest>.64)c=t<.42?new Color4(.26,.37,.27,1):new Color4(.29,.41,.24,1);
  else if(meadow>.60)c=new Color4(.48,.54,.30,1);
  else if(m>.70)c=new Color4(.32,.46,.28,1);
  else if(m>.48)c=new Color4(.39,.49,.29,1);
  else c=new Color4(.46,.50,.31,1);
  const micro=(fbm(x*.011+241,z*.011-107,3)-.5)*.075;
  const warm=(fbm(x*.0028-91,z*.0028+54,3)-.5)*.035;
...
export class WorldStreamer{
  ...
  constructor(scene:Scene){
    this.scene=scene;
    this.terrainMaterial=new StandardMaterial('terrainMat',scene);this.terrainMaterial.diffuseColor=Color3.White();this.terrainMaterial.specularColor=new Color3(.012,.015,.009);this.terrainMaterial.specularPower=22;this.terrainMaterial.backFaceCulling=false;this.terrainMaterial.emissiveColor=new Color3(.030,.035,.021);this.terrainMaterial.ambientColor=new Color3(.20,.23,.16);
    ...
    this.firDark=this.makeFir('firDark',new Color3(.072,.175,.102),0);
    this.firBlue=this.makeFir('firBlue',new Color3(.090,.205,.165),1);
    this.oak=this.makeBroad('oak',new Color3(.155,.280,.112),0);
    this.aspen=this.makeBroad('aspen',new Color3(.205,.315,.125),1);
    ...
    try{
      ...
      for(const m of r.meshes){
        ...
        if(m.material instanceof PBRMaterial){
          m.material.roughness=Math.max(m.material.roughness??.86,.86);m.material.metallic=Math.min(m.material.metallic??0,.012);m.material.environmentIntensity=.54;
          if(lower.includes('fir'))m.material.albedoColor=new Color3(.075,.19,.11);else if(lower.includes('broad'))m.material.albedoColor=new Color3(.17,.29,.115);else m.material.albedoColor=new Color3(.34,.35,.32);
          if(m.material.albedoTexture)m.material.albedoTexture.level=.28;
        }
        if(m.material instanceof StandardMaterial){m.material.specularColor=new Color3(.008,.011,.007);if(lower.includes('fir'))m.material.diffuseColor=new Color3(.075,.19,.11);else if(lower.includes('broad'))m.material.diffuseColor=new Color3(.17,.29,.115);else m.material.diffuseColor=new Color3(.34,.35,.32);}
      }
  ...
  private createChunk(cx:number,cz:number,near:boolean){
    ...
    const instances:InstancedMesh[]=[];const treeCount=this.scatter(cx,cz,root,instances,near?1:.29);this.chunks.set(key,{key,cx,cz,root,instances,treeCount});
  }
  private createRiver(cx:number){
    const seg=96,positions:number[]=[],indices:number[]=[],uvs:number[]=[],x0=cx*CHUNK-HALF_CHUNK;
    for(let i=0;i<seg;i++){
  ...
  private createLakes(cx:number,cz:number,root:TransformNode){
    ...
    for(let lz=z0;lz<=z1;lz++)for(let lx=l0;lx<=l1;lx++){
      ...
      const seg=56,positions=[lake.x,lake.level+.24,lake.z],indices:number[]=[],uvs=[0,1];
  ...
  private scatter(cx:number,cz:number,root:TransformNode,out:InstancedMesh[],density:number){
    ...
    for(let i=0;i<candidates;i++){
      ...
      if(authored)kind=(kind==='oak'||kind==='aspen')?'authoredBroad':'authoredFir';if(hash(i*71+cx,cz-i*17)>.972)kind='authoredRock';
  ...
  private makeFir(name:string,color:Color3,variant:number){
    const bark=new StandardMaterial(`${name}_bark`,this.scene);bark.diffuseColor=new Color3(.21,.13,.067);bark.specularColor=Color3.Black();bark.ambientColor=bark.diffuseColor.scale(.22);
    const leaf=new StandardMaterial(`${name}_leaf`,this.scene);leaf.diffuseColor=color;leaf.specularColor=new Color3(.006,.009,.005);leaf.emissiveColor=color.scale(.012);leaf.ambientColor=color.scale(.22);
    const parts:Mesh[]=[];const height=8.7+variant*.7,trunk=MeshBuilder.CreateCylinder(`${name}_trunk`,{height,diameterTop:.42,diameterBottom:.86,tessellation:8},this.scene);trunk.position.y=height*.5;trunk.material=bark;parts.push(trunk);
    ...
    tiers.forEach((t,i)=>{const crown=MeshBuilder.CreateCylinder(`${name}_tier${i}`,{height:t.h,diameterTop:.16,diameterBottom:t.r*2,tessellation:9},this.scene);crown.position.set((i%2?-.13:.11)*(variant+1),t.y,(i%2?.09:-.08)*(variant+1));crown.scaling.z=.88+(i%3)*.055;crown.rotation.y=i*.69;crown.material=leaf;parts.push(crown);});const merged=Mesh.MergeMeshes(parts,true,true,undefined,false,true)!;merged.name=name;return merged;
  }
  private makeBroad(name:string,color:Color3,variant:number){
    const bark=new StandardMaterial(`${name}_bark`,this.scene);bark.diffuseColor=variant?new Color3(.33,.28,.20):new Color3(.25,.16,.085);bark.specularColor=Color3.Black();bark.ambientColor=bark.diffuseColor.scale(.20);
    const leaf=new StandardMaterial(`${name}_leaf`,this.scene);leaf.diffuseColor=color;leaf.specularColor=new Color3(.007,.010,.005);leaf.emissiveColor=color.scale(.010);leaf.ambientColor=color.scale(.20);
    ...
    blobs.forEach((b,i)=>{const crown=MeshBuilder.CreateIcoSphere(`${name}_crown${i}`,{radius:1,subdivisions:1},this.scene);crown.position.set(b[0],b[2],b[1]);crown.scaling.set(b[3],b[4],b[5]);crown.rotation.set((i%2-.5)*.20,i*.79,(i%3-1)*.13);crown.material=leaf;parts.push(crown);});const merged=Mesh.MergeMeshes(parts,true,true,undefined,false,true)!;merged.name=name;return merged;
  }
  private makeShrub(name:string){const leaf=new StandardMaterial(`${name}_leaf`,this.scene);leaf.diffuseColor=new Color3(.145,.265,.105);leaf.specularColor=Color3.Black();leaf.emissiveColor=leaf.diffuseColor.scale(.010);leaf.ambientColor=leaf.diffuseColor.scale(.20);const parts:Mesh[]=[];const blobs=[[-.65,0,.66,.86,.65,.72],[.54,.12,.72,.78,.72,.84],[0,-.44,.58,.92,.58,.72],[.05,.28,1.12,.70,.72,.68]];blobs.forEach((b,i)=>{const p=MeshBuilder.CreateIcoSphere(`${name}_${i}`,{radius:1,subdivisions:1},this.scene);p.position.set(b[0],b[2],b[1]);p.scaling.set(b[3],b[4],b[5]);p.rotation.y=i*.77;p.material=leaf;parts.push(p);});const merged=Mesh.MergeMeshes(parts,true,true,undefined,false,true)!;merged.name=name;return merged;}
  private makeRock(name:string){const rock=MeshBuilder.CreateIcoSphere(name,{radius:2.0,subdivisions:1},this.scene);rock.scaling.set(1.55,.72,1.18);rock.rotation.set(.12,.35,-.08);const m=new PBRMaterial(`${name}_mat`,this.scene);m.albedoColor=new Color3(.34,.35,.32);m.roughness=.99;m.metallic=.008;rock.material=m;return rock;}
  private makeFlower(name:string){const stem=MeshBuilder.CreateCylinder(`${name}_stem`,{height:.90,diameter:.075,tessellation:5},this.scene);stem.position.y=.45;const bloom=MeshBuilder.CreateIcoSphere(`${name}_bloom`,{radius:.21,subdivisions:1},this.scene);bloom.position.y=.99;const sm=new StandardMaterial(`${name}_stemMat`,this.scene);sm.diffuseColor=new Color3(.14,.27,.12);sm.specularColor=Color3.Black();stem.material=sm;const bm=new StandardMaterial(`${name}_bloomMat`,this.scene);bm.diffuseColor=new Color3(.66,.47,.20);bm.specularColor=Color3.Black();bloom.material=bm;const merged=Mesh.MergeMeshes([stem,bloom],true,true,undefined,false,true)!;merged.name=name;return merged;}
}
