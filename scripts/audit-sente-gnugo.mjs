...
function isEyeFill(game, x, y, color) {
  ...
  for (const neighbor of neighbors) {
    if (game.board[neighbor.y * game.size + neighbor.x] === color) friendlyOrEdge += 1;
  }
  ...
  for (const [dx, dy] of [[-1, -1], [1, -1], [-1, 1], [1, 1]]) {
    ...
    else if (game.board[py * game.size + px] === opponent) hostileDiagonals += 1;
  }
...
async function engineMove(engine, game, level, seed, readOverride = null) {
  ...
  for (let index = 0; index < plan.length; index += 1) {
    ...
    const output = await engine.play(read.seed, buildSgf(compactGame(game), read.transform));
...
async function setupProbe(engine, name, sgf, color, expected, forbidden = null, seed = 1) {
  const output = await engine.play(seed, sgf);
  ...
  check(name, passed, {
    ...
    expected: expected ? coordinate(expected) : undefined,
    forbidden: forbidden ? coordinate(forbidden) : undefined
  });
}
...
async function playObservedGame(engine, options) {
  ...
  for (let ply = 0; ply < 120 && game.phase === 'playing'; ply += 1) {
    ...
    const move = engineTurn
      ? await engineMove(engine, game, options.level || 'steady', options.seed + ply * 7919, options.auditReads || 1)
      : scriptedMove(game, options.opponentStyle || 'expansion', rng);

    if (!move || move.pass) {
      if (firstPass === null) firstPass = game.moveNumber;
      const color = game.turn;
      passTurn(game);
      moveLog.push({ number: game.moveNumber, color, move: 'pass', engine: engineTurn });
    } else {
      ...
      if (!inspection.legal) {
        const color = game.turn;
        passTurn(game);
        moveLog.push({ number: game.moveNumber, color, move: 'illegal→pass', engine: engineTurn });
      } else {
        const color = game.turn;
...
await setupProbe(engine, 'Black captures a stone in atari', captureBlack, BLACK, { x: 4, y: 5 }, null, 11);
await setupProbe(engine, 'Black saves a stone in atari', saveBlack, BLACK, { x: 4, y: 5 }, null, 12);
await setupProbe(engine, 'White captures a stone in atari', captureWhite, WHITE, { x: 4, y: 5 }, null, 13);
await setupProbe(engine, 'White saves a stone in atari', saveWhite, WHITE, { x: 4, y: 5 }, null, 14);
...
for (let seed = 20; seed < 28; seed += 1) await setupProbe(engine, `Black does not fill own eye seed ${seed}`, blackEye, BLACK, null, { x: 4, y: 4 }, seed);
for (let seed = 30; seed < 38; seed += 1) await setupProbe(engine, `White does not fill own eye seed ${seed}`, whiteEye, WHITE, null, { x: 4, y: 4 }, seed);
...
for (const game of games) {
  ...
  check(`${game.name}: engine does not mainly clump`, game.densePeacefulRate < 0.38, {
    rate: game.densePeacefulRate,
    dense: game.densePeaceful,
    engineMoves: game.engineMoves
  });
...
const report = {
  ...
  startedAt,
  engine: { name: 'GNU Go', version: engine.version, upstreamCommit: '382df5a9b14b62ea451012ec7d2e81c61162e037', isolatedRuntimePerRead: true },
...
const markdown = [
  ...
  `Engine: GNU Go ${engine.version}; clean runtime per independent reading`,
...
if (failures.length) throw new Error(`SENTE AI audit has ${failures.length} failure(s). See apps/sente/AI_AUDIT.md.`);
