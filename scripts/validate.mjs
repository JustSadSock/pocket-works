...
async function readJson(relativePath, label = relativePath) {
  ...
  catch (error) { fail(`${label} is not valid JSON: ${error.message}`); return null; }
}
...
async function validateHtml(label, relativePath) {
  let html;
  try { html = await readText(relativePath); }
  catch (error) { fail(`${label} could not be read: ${error.message}`); return; }
  ...
  if (!/name=["']viewport["'][^>]*viewport-fit=cover/i.test(html)) fail(`${label} viewport meta must include viewport-fit=cover`);
}
...
async function validateManifest(label, relativePath, expected) {
  ...
  for (const key of ['id', 'name', 'short_name', 'start_url', 'scope', 'display', 'background_color', 'theme_color']) requireString(manifest, key, label);
  ...
  if (!['standalone', 'fullscreen', 'minimal-ui'].includes(manifest.display)) fail(`${label}.display must be standalone, fullscreen or minimal-ui`);
  ...
  for (const [index, icon] of manifest.icons.entries()) {
    ...
    if (!(await exists(path.join(root, iconPath)))) fail(`${label}.icons[${index}] points to missing file ${icon.src}`);
  }
}

async function validateServiceWorker(label, relativePath, expectedPrefix, runtime = 'quick') {
  let source;
  try { source = await readText(relativePath); }
  catch (error) { fail(`${label} could not be read: ${error.message}`); return; }

  if (runtime === 'enhanced') {
    if (source.trim().length < 100) fail(`${label} generated Workbox worker is unexpectedly small`);
    return;
  }

  for (const eventName of ['install', 'activate', 'fetch']) {
    if (!source.includes(`addEventListener('${eventName}'`) && !source.includes(`addEventListener("${eventName}"`)) fail(`${label} must handle the ${eventName} event`);
  }

  const cacheMatch = source.match(/(?:const|let)\s+CACHE_NAME\s*=\s*['"`]([^'"`]+)['"`]/);
  if (!cacheMatch) { fail(`${label} must declare a static CACHE_NAME string`); return; }
  const cacheName = cacheMatch[1];
  if (!cacheName.startsWith(expectedPrefix)) fail(`${label} CACHE_NAME must start with ${expectedPrefix}`);
...
try {
  ...
  if (!validatesServiceWorkerRegistration(launcherSource, launcherHtml)) fail('root launcher must register ./sw.js directly or through the managed update script');
} catch (error) {
