...
const deployCommand = 'npm run registry:build && npm run prepare:site && npm run validate:site';
const fullCiCommand = 'npm run prepare:sente-engine && npm run test:sente-ai && npm run registry:build && npm run health && npm run prepare:site && npm run validate:site';
...
for (const forbidden of ['prepare:sente-engine', 'test:sente-ai', 'health', 'validate:all', 'test:forge', 'test:enhanced', 'build:enhanced']) {
  if ((scripts['deploy:site'] || '').includes(forbidden)) {
    errors.push(`deploy:site must not include heavy step ${forbidden}`);
  }
}
...
console.log('Deployment pipeline separation is valid: Netlify is fast, GitHub CI is exhaustive.');
