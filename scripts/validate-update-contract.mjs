...
function validateCoherentWorker(source,label,expected){requireIncludes(source,['GET_UPDATE_INFO','SKIP_WAITING','cache:','REQUIRED','OPTIONAL'],label);validateMetadata(source,label,expected)}
function validateFresh(source,label,{passThroughApps=false}={}){requireIncludes(source,['CACHE_PROTOCOL','precacheFreshShell','cache:','__pw_build','networkFirstFresh','SHELL_KEYS'],label);if(source.includes('cache.addAll('))fail(`${label} must not use cache.addAll`);const protocol=Number(source.match(/const CACHE_PROTOCOL\s*=\s*(\d+)/)?.[1]);if(!Number.isInteger(protocol)||protocol<2)fail(`${label} cache protocol must be at least 2`);if(passThroughApps)requireIncludes(source,['APPLICATIONS_PATH','startsWith(APPLICATIONS_PATH)'],label)}
