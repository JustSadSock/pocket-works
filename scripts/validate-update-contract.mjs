...
function validateMetadata(source, label, expected) {
  ...
  if (expected.changelog) {
    if (JSON.stringify(releaseNotes) !== JSON.stringify(expected.changelog)) fail(`${label} RELEASE_NOTES must match app.config.json changelog`);
...
requireIncludes(enhancedTemplateMain, ['registerEnhancedUpdate', "version: '__APP_VERSION__'", 'releaseNotes: __APP_CHANGELOG_JSON__'], 'apps/_enhanced-template/source/main.ts');
requireIncludes(enhancedTemplateWorker, ['precacheAndRoute', 'GET_UPDATE_INFO', 'SKIP_WAITING'], 'apps/_enhanced-template/source/sw.ts');

const configs = await collectAppConfigs(root);
for (const config of configs) {
  const directory = `apps/${config.slug}`;
  if (runtimeForConfig(config) === 'enhanced') {
    const main = await read(`${directory}/source/main.ts`);
    const worker = await read(`${directory}/source/sw.ts`);
    requireIncludes(main, ['registerEnhancedUpdate', `version: '${config.version}'`], `${directory}/source/main.ts`);
    validateEnhancedWorker(worker, `${directory}/source/sw.ts`, config);
  } else {
...
console.log(`Managed update contract passed for Quick and Enhanced runtimes plus ${configs.length} published app${configs.length === 1 ? '' : 's'}.`);
