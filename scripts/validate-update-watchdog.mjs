...
const [launcher, index, rootWorker, prepareSite, vetrolomWorker, vetrolomConfig] = await Promise.all([
  read('launcher-update-all-v2.js'),
  read('index.html'),
  read('sw.js'),
...
for (const token of ['APP_TIMEOUT','withTimeout(updateApplicationCore','full update','timedOut:true','skipped —','UPDATE_CONCURRENCY=2']) requireToken(launcher, token, 'launcher watchdog');
for (const source of [index, rootWorker, prepareSite]) requireToken(source, 'launcher-update-all-v2.js', 'launcher deployment');
...
console.log('Bulk update watchdog and Vetrolom resilient precache contract are valid.');