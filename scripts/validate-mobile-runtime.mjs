...
requireIncludes(runtimeCss, [
  ...
  '.is-app-scroll-locked',
  '[data-native-press].is-pressed'
...
requireIncludes(runtimeJs, [
  ...
  'export function setDocumentScrollLocked',
...
const appTargets = [
  ...
  ...registry.map((app) => ({ slug: app.slug, registered: true }))
];
...
for (const target of appTargets) {
  ...
  requireIncludes(appJs, [
    '../../shared/mobile-runtime.js',
    'installMobileRuntime'
  ], `${directory}/app.js`);

  requireIncludes(serviceWorker, [
    '../../shared/mobile-runtime.css',
    '../../shared/mobile-runtime.js'
  ], `${directory}/sw.js`);

  if (target.registered && !html.includes('data-native-press')) {
    fail(`${directory}/index.html must expose at least one immediate press-feedback control`);
  }
}
...
console.log(`Mobile runtime validation passed for the shared runtime, template and ${registry.length} registered app${registry.length === 1 ? '' : 's'}.`);
