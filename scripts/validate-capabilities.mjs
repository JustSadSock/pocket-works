...
requireFragments(sources.get('shared/capabilities/motion.js'), [
...
requireFragments(sources.get('shared/capabilities/storage.js'), [
  'export function createVersionedStore',
  'export function estimateNamespaceBytes',
  'export function clearNamespace',
  'migrations',
  'validate'
], 'shared/capabilities/storage.js');

requireFragments(sources.get('shared/capabilities/transfer.js'), [
...
requireFragments(sources.get('shared/capabilities/audio.js'), [
...
requireFragments(sources.get('shared/capabilities/device.js'), [
  'export function getDeviceCapabilities',
  'export async function watchOrientation',
  'export async function watchMotion',
  'export async function toggleFullscreen',
  'export async function lockOrientation'
], 'shared/capabilities/device.js');

requireFragments(sources.get('shared/capabilities/diagnostics.js'), [
  'export function createFpsProbe',
  'export function createErrorCollector',
  'export async function collectDiagnostics',
  'export async function clearOwnedCaches',
  'clearOwnedStorage'
], 'shared/capabilities/diagnostics.js');

requireFragments(sources.get('shared/workshop-mode.js'), [
  'export function createWorkshopMode',
  'setDocumentScrollLocked',
  'data-workshop-trigger',
  'Tap again to clear',
  'Tap again to reset',
  'workshopopen',
  'workshopclose'
...
requireFragments(sources.get('shared/workshop-mode.css'), [
  '.workshop-mode',
...
for (const [file, source] of sources) {
  if (/https?:\/\//.test(source)) fail(`${file} must not depend on a remote runtime asset`);
}
...
requireFragments(templateIndex, [
  '../../shared/workshop-mode.css',
  'data-workshop-trigger'
], 'apps/_template/index.html');
requireFragments(templateApp, [
  "from '../../shared/capabilities/storage.js'",
  "from '../../shared/workshop-mode.js'",
  'createVersionedStore',
  'createWorkshopMode'
], 'apps/_template/app.js');
for (const offlineFile of requiredOfflineFiles) {
  if (!templateWorker.includes(offlineFile)) fail(`apps/_template/sw.js must cache ${offlineFile}`);
}
...
for (const config of configs) {
  ...
  requireFragments(app, [
    ...
    `cachePrefix: '${config.slug}-'`
  ], `${directory}/app.js`);

  if (app.includes("navigator.serviceWorker.register('./sw.js')")) {
    fail(`${directory}/app.js must leave Service Worker registration to update-manager`);
  }

  for (const offlineFile of requiredOfflineFiles) {
    if (!worker.includes(offlineFile)) fail(`${directory}/sw.js must cache ${offlineFile}`);
  }
}
...
console.log(`Shared capabilities and Workshop Mode passed for the template and ${configs.length} published app${configs.length === 1 ? '' : 's'}.`);
