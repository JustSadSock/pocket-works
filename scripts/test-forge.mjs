...
import { APP_PRESETS } from './app-config.mjs';
...
try {
  for (const [index, preset] of APP_PRESETS.entries()) {
  ...
  console.log(`Pocket Forge smoke test passed for ${APP_PRESETS.length} presets.`);
} finally {
  for (const slug of generatedSlugs) {
    await rm(path.join(root, 'apps', slug), { recursive: true, force: true });
  }
